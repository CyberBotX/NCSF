...
public class Track : NCSFCommon.Track
{
		...
		while (this.wait == 0 && !this.Flags.HasFlag(TrackFlag.NoteFinishWait))
		{
			...
			if ((cmd & 0x80) == 0)
			{
				...
				if (runCmd)
				{
					...
					if ((this.Player as Player)!.DoNotes)
						...
						case SSEQCommand.OpenTrack:
						{
							...
							if (runCmd)
							{
								...
								if (newTrack is not null && newTrack != this)
								{
									...
									(this.Player as Player)!.SetUsedTrack(par, true);
								}
							}
							break;
						}
	...
	public static List<(uint position, ValueType valueType, ushort patch)> GetPatches(NCSFCommon.NC.SSEQ sseq) =>
		Track.GetPatches(sseq.Data);

	public static List<(uint position, ValueType valueType, ushort patch)> GetPatches(ReadOnlyMemory<byte> data)
	{
		List<(uint position, ValueType valueType, ushort patch)> patches = [];
		...
		try
		{
			while (track.CurrentPos < dataLength)
			{
				ValueType? valueType = null;

				byte cmd = track.ReadU8();
				...
				{
					_ = track.ReadU8(); // par
					_ = track.ParseValue(valueType ?? ValueType.VLV); // length
					if (finishedTracks == 0 && patches.Count == 0)
						usePatch0 = true;
					continue;
				}
				...
				switch (cmd & 0xF0)
				{
					case 0x80:
					{
						int par = track.ParseValue(valueType ?? ValueType.VLV);
						switch (cmd.ToEnum<SSEQCommand>())
						{
							case SSEQCommand.Patch:
								patches.Add(((uint)track.CurrentPos, valueType ?? ValueType.VLV, (ushort)par));
								break;
						}
						break;
					}
					...
					case 0xC0:
					case 0xD0:
						_ = track.ParseValue(valueType ?? ValueType.U8); // par
						break;
					case 0xE0:
						_ = track.ParseValue(valueType ?? ValueType.U16); // par
						break;
					case 0xB0:
						_ = track.ReadU8(); // varNum
						_ = track.ParseValue(valueType ?? ValueType.U16); // par
						break;
					case 0xF0:
						switch (cmd.ToEnum<SSEQCommand>())
		...
		// such as badly written sequences with an incorrect number of tracks and/or extra bytes after the end
		...
		if (usePatch0)
			patches.Add((uint.MaxValue, ValueType.VLV, 0));
		return patches;
	}
}
